...
################################################################
def readgalList(table, skyPatch='none', R_min=6.0, R_max=100000000.):
...
#######################################################################
	...
	if restrict and ang12 <= 1.0*max(r1,r2) and max(r1,r2)<6:
	  if abs(v1-v2) <= max(2*Sigquad,3.0*max(sig1,sig2)):
	      Bol = True
...
def addGalGroup(G_list, galList, ignore_list=[]):
    ...
    #for gal in galList:
      #if gal.id == 626659: print gal.Vls
    ...
    for i in indices[::-1]:
	    ...
	    q1[np.where(LL<=G_list[i][0].gl+10*r)] = 1
	    q2[np.where(LL>=G_list[i][0].gl-10*r)] = 1
	    q3[np.where(BB<=G_list[i][0].gb+10*r)] = 1
	    q4[np.where(BB>=G_list[i][0].gb-10*r)] = 1
	    qq = q1+q2+q3+q4
	    subgalList =[]
	    gal_indices = np.where(qq==4)
	    for index in gal_indices[0]:
		...
		Bol = True
		if  subgalList[p].inGroup == 1 or (subgalList[p].id in ignore_list):
		          Bol = False  #  It's already in a group
		...
		if Dist1 <= 1: Dist1 = 1.
		#if G_list[i][0].nest == 71866: print "Ahan", sig_p, G_list[i][0].Vls
		#if subgalList[p].id == 626659 and  G_list[i][0].nest == 71866: print subgalList[p].Vls, ang12, r
	        if Bol==True  and ang12 <= 1.05*(180./pi)*atan(1.1*G_list[i][0].R_2t2/(Dist1)) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 3.0*sig_p:
...
#################################################################
def IndividualGal(G_list, galList_org, coeff=1.0, ignore_list=[]):
...
if __name__ == '__main__':
  ...
  if len(sys.argv) < 2:
    ...
    print "\nPossible options: virgo, fornax, north and south" 
  ...
  for iter in range(11):
        ...
        print "working on grouping ..."
	##  Finding groups based on the criteria
	G_list = []
        ignore_list=[41618, 40001, 42447, 40240, 40851]
        for qp in range(0,3):
	   G_list += IndividualGal(G_list, galList, ignore_list = ignore_list)
	...
	for qp in range(0,20):
	  ...
	  #group = G_list[i]
	  #groupHeader = group[0]
	  #if groupHeader.nest == 39659:  # virgo-W
	    #for galaxy in group[1:]:
	      
	      #if galaxy.sgb > -10 and galaxy.Vls <2000 and (galaxy.dcf2 < 25):
		#galaxy.inGroup = 0

	      #else: 
		#newGroup_list.append(galaxy)
	    #break
	#root = LoosgalListJoint(newGroup_list, grID = 200000000)
	#G_list[i] = NodeLeaf(root)
        ...
        ###
        #G1 = IndividualGal(G_list, galList)
        #G_list += G1
        
        
        ## we add, so the newly formed groups have a chance to absorb 
        #G_list = addGalGroup(G_list, galList, ignore_list = ignore_list)
        ###
...
####################################################################
	...
	for galaxy in galList:
	  if galaxy.inGroup == 0:
        ...
        if cluster == 'virgo':
	  virgoPlot(G_list, galList, R_max=R_max)
	elif cluster == 'virgo_w':
	  virgoPlot(G_list, galList, R_max=R_max, sgl_min=105, sgl_max=117, sgb_min=-10, sgb_max=2)
	else: fornaxPlot(G_list, galList)
	#plt.savefig('virgo_w.v01.ps')


        if cluster == 'virgo':
	  groupWrite('virgo.iter.'+str(int(iter))+'.v23.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.iter.'+str(int(iter))+'.v23.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.iter.'+str(int(iter))+'.v23.group', G_list, galList)
        else:
	  groupWrite('south.iter.'+str(int(iter))+'.v23.group', G_list, galList)

        #groupWrite('virgo_w.v01.group', G_list, galList)
	
	for galaxy in galList: galaxy.inGroup = 0
        
        
        if cluster == 'virgo':
	   ## the best view and size happens in eps format
           plt.savefig('virgo.iter.'+str(int(iter))+'.v23.eps', dpi=600)
           print 'done with ...', 'virgo.iter.'+str(int(iter))+'.v23.eps'
        elif cluster == 'fornax':
	   plt.savefig('fornax.iter.'+str(int(iter))+'.v23.eps', dpi=600) 
	   print 'done with ...', 'fornax.iter.'+str(int(iter))+'.v23.eps'
