...
if __name__ == '__main__':
  ...
  for i in range(0, N_galaxies):
     ...
     #if  pgc[i]==4075680: print "   checkpoint:    ", SGL0[i], SGL0p[i]
     if Ty0[i]!= -100000: 
       Ty[i] =  Ty0[i]   # LEDA
       Ty_source[i] = 'LEDA'
       LEDAflag[i] = 0
     ...
     for ppggcc in [9140, 166096, 166167, 2815820, 5056918]:
         if pgc[i]== ppggcc:
	   print ppggcc, Vls[i], '  ', Vls_source[i]
...
#################################################################################  
#################################################################################  
     if Vls[i] <= 0 or Vls[i] > 4000: flag[i] += 1
     if gb[i] > 0: flag[i] += 1    # north < 0  or south > 0
     if LEDAflag[i] == 1 and BB[i]<16: flag[i] += 1
     if SelectionFlag[i] == 0 : flag[i] += 1
     if flag[i]==0 and Vls[i] > 1000 : LEDAflag[i] = 0  
     #if flag[i]==0 and (Vls[i] <= 300 or Vls[i] > 1000) : LEDAflag[i] = 0 
     if flag[i]!=0: LEDAflag[i] = 0   # I am interested in those galaxies which you don't use (i.e. flag!=0)
...
#################################################################################  
  ...
  if len(flag[0]) != 0 :
      ...
      myTable.write('AllSky.south.v9.csv', format='ascii.fixed_width',delimiter=',', bookend=False)
      
#################################################################################  
#################################################################################  
  # Select those galaxies for which the LEDAflag = 0
  # These are those galxies in our catalog with 0<Vls<100 btc>16 and only LEDA information.
  # Since these objects are fairly nearby, we are gonna check them manually to make sure that they are not stars
  flag = np.where(LEDAflag==1)
  #print 'flag:  ... .... : ', flag
  if len(flag[0]) != 0 :
      pgc_file = pgc[flag]
      gl_file = gl[flag]
      ...
      myTable.write('AllSky.south.v9.LEDAflag.csv', format='ascii.fixed_width',delimiter=',', bookend=False) 

  else: print "No LEDA flag file ...."
