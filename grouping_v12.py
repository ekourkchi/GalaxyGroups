...
def readgalList(table, skyPatch='none', R_min=6.0, R_max=100000000., sgl_min=0, sgl_max=0, sgb_min=0, sgb_max=0, ignore_list=[]):
  ...
  for i in range(N_galaxies):
    ...
    if fornax:
      ...
      if Vls[i] < 4000 and sgl[i]<117 and sgl[i]>105 and sgb[i]>-10 and sgb[i]<2 and ang12 > R_min and ang12 < R_max:
        Bool = True
    elif  manual:
      if Vls[i] < 4000  and sgl[i]<sgl_max and sgl[i]>sgl_min and sgb[i]>sgb_min and sgb[i]<sgb_max:
        Bool = True
    else:
      if Vls[i] < 4000: 
	  Bool = True
    if Bool:         
	   ...
	   node.Vhelio = Vhelio[i]
           ...
           if id[i] in ignore_list or Ks[i] < 0:
...
#################################################################
def IndividualGal(galList_org, coeff=1.0):
...
####################################################################
	...
	for galaxy in galList:
	  if galaxy.inGroup == 0:
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.iter.'+str(int(iter))+'.v25.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.iter.'+str(int(iter))+'.v25.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.iter.'+str(int(iter))+'.v25.group', G_list, galList)
        elif cluster == 'south':
	  groupWrite('south.iter.'+str(int(iter))+'.v25.group', G_list, galList)
        elif cluster == 'manual':
	  groupWrite('manual.iter.'+str(int(iter))+'.v25.group', G_list, galList)
