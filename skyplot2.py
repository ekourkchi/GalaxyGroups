...
class PointBrowser(object):
    ...
    def single_info(self, dataind):
	  ...
	  sgx, sgy, sgz = xyz_list(self.sgl_gal[ind], self.sgb_gal[ind], self.Vls_gal[ind], self.dist_gal[ind], self.mDist_gal[ind], self.flag_gal[ind])
...
def ClusterPlot(id, ra, dec, Vls, R_2t2, r1t_lum, dist, nest, flag, parent, mode='none'):
  ...
  while gr == 5:
    ...
    if len(X_gal) > 0:
        point, = plt.plot(X_gal, Y_gal, 's', markersize = 3, color=colour, markeredgecolor = 'black')
        parent.list_point.append([point, (Red, Green, Blue), colour, Vls_grp, True, False])    
	# ----------------------------------------------------------------------------^ means it's NOT a galaxy        
    if len(X_grp) > 0:
        point, = plt.plot(X_grp, Y_grp, 'o', markersize = 5, color=colour, markeredgecolor = colour)
        parent.list_point.append([point, (Red, Green, Blue), colour, Vls_grp, True, True])           
	# ----------------------------------------------------------------------------^ means it's a group       
...
class Plot_page(tk.Frame):
  ...
  def show_plot(self):
	...
	self.line, = plt.plot(A, B, 'o', markersize = 1, color='white', markeredgecolor='white', picker=5)   # color='#696969'
	...
	for i in range(len(id_gal)):
	  ...
	  if flag_gal[i] == 3 or flag_gal[i] == 0: 
	     l, = plt.plot([A[i]], [B[i]], 'x', markersize = 2.5, color='white', markeredgecolor=color_table(Vls_gal[i]))
	     self.lline.append([l,color_table(Vls_gal[i]),Vls_gal[i], True, False, '#696969']) 
	     # ---------------------------------------------------------------^ means it's a galaxy
	  
	  if flag_gal[i] == 4 or flag_gal[i] == 2: 
	     l, = plt.plot([A[i]], [B[i]], 'o', markersize = 3.5, color='white', markeredgecolor=color_table(Vls_gal[i]))
	     self.lline.append([l,color_table(Vls_gal[i]),Vls_gal[i], True, True, (Red, Green, Blue)])
	     # ---------------------------------------------------------------^ means it's a group
	...
	ClusterPlot(id, A_cluster, B_cluster, Vls, R_2t2, r1t_lum, dist, nest, flag, self, mode=mode)
	...
	def cng_color(event):
	  ...
	  if self.color_bts[p][1]:
	    ...
	    for obj in self.list_point:
	      if vel_limits(p, obj[3]) and obj[4]:
		 if obj[5]: obj[0].set_visible(True)
		 if not obj[5] and gal_on: obj[0].set_visible(True)
	    for obj in self.lline:
	      if vel_limits(p, obj[2]) and obj[3]:
		 if obj[4]: obj[0].set_visible(True)
		 if not obj[4] and gal_on: obj[0].set_visible(True)	
	...
	###############################
	rax0 = axes([0.85, 0.03, 0.11, 0.07], axisbg='tan')
        self.radio_gal = RadioButtons(rax0, ('On', 'Off'), active=0)
        annotate('Galaxies on/off ?', (0.86,0.11), xycoords='figure fraction', size=10, color='black')	
	  ...
	  if label == 'On':
	    gal_on = True
	    for obj in self.lline:
	       if not obj[4]: obj[0].set_visible(True)
	    
	    for obj in self.list_point:
	       if not obj[5]: obj[0].set_visible(True)
        
	  if label == 'Off':
	    gal_on = False
	    for obj in self.lline:
		if not obj[4]: obj[0].set_visible(False)
	    
	    for obj in self.list_point:
	        if not obj[5]: obj[0].set_visible(False)
	      
        self.radio_gal.on_clicked(galfunc)
        #######################  	
	###############################
	rax = axes([0.85, 0.14, 0.11, 0.11], axisbg='tan')
        self.radio = RadioButtons(rax, ('No', 'Yes'), active=0)
        annotate('Random Colors ?', (0.86,0.26), xycoords='figure fraction', size=10, color='black')
	   ...
	   global gal_on
		 ...
		 obj[0].set_markeredgecolor('black')
	       else: obj[0].set_markeredgecolor(obj[1])
	   ...
	   if label == 'No':
	     ...
	     for obj in self.list_point:
	       ...
	       if not obj[5]: 
		 obj[0].set_markeredgecolor('black')
	       else: obj[0].set_markeredgecolor(obj[2])	       
	     for obj in self.lline:
...
if __name__ == '__main__':
	  ...
	  print >> sys.stderr, "Use \"python "+sys.argv[0]+" -h\" for help ... (or 'skyplot2 -h') \n"
	  exit(1)
