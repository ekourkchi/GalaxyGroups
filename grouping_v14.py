...
def galJoint(galNode1, galNode2, ID):
   ...
   if newNode.sumDist !=0 and newNode.sumError!=0:
        mDM = newNode.sumDist/newNode.sumError
        meDM = sqrt(1/newNode.sumError)
        meanDist = 10**(mDM/5.-5.)
        meanDistErr = (0.2*log(10.))*meDM
   else:
        mDM = 0.
        meDM = 0.
        meanDist = 0.
...
class GalxyNode:
  ...
  def __init__(self, id, gl, gb, sgl, sgb, Vls, Ks, Ty, dcf2, ed, dm, edm):
    ...
    if dcf2!=0 and ed!=0:
        ...
        self.sumDist = 1. * dm / (edm**2)
        self.sumError = 1. / (edm**2)
    else:
        self.sumDist = 0.
        self.sumError = 0.
...
################################################################
def readgalList(table, skyPatch='none', R_min=6.0, R_max=100000000., sgl_min=0, sgl_max=0, sgb_min=0, sgb_max=0, ignore_list=[]):
  ...
  for i in range(N_galaxies):
    ...
    Bool = False
    if fornax:
      if Vls[i] < 4000 and sgl[i]<303 and sgl[i]>240 and sgb[i]>-57 and sgb[i]<-20:
        Bool = True
    
    elif virgo_w:
      ang12 = (180./pi)*angle(sgl[i], sgb[i], sglV, sgbV)
      if Vls[i] < 4000 and sgl[i]<117 and sgl[i]>105 and sgb[i]>-10 and sgb[i]<2 and ang12 > R_min and ang12 < R_max and ang12 > R_min and ang12 < R_max:
        Bool = True
    elif  manual:
      if Vls[i] < 4000  and sgl[i]<sgl_max and sgl[i]>sgl_min and sgb[i]>sgb_min and sgb[i]<sgb_max and ang12 > R_min and ang12 < R_max:
        Bool = True
    else:
      if Vls[i] < 4000 and ang12 > R_min and ang12 < R_max: 
	  Bool = True
    if Bool:         
           if id[i] == 166096:
	     dcf2[i] = 9.3
...
#######################################################################
	...
	if (idd1==13418 and  idd2==12651) or (idd1==12651 and  idd2==13418):
	  Bol = False  
	if (idd1==39659 and  idd2==38964) or (idd1==38964 and  idd2==39659):
	  Bol = False    	
	if (idd1==39659 and  idd2==39483) or (idd1==39483 and  idd2==39659):
	  Bol = True    	
...
def addGalGroup(G_list, galList):
    ...
    for entity in singles:
		...
		if gal.id == 39659: is_virgo_w = True
		...
		#print "Esn TEst: ", G_list[i][0].Vls, sig_p, " <> ", entity[0].id, entity[0].Vls
	      ...
	      if ang12 <= 2*r:
		    ...
		    if is_virgo_w and entity[0].Vls<1450 and entity[0].sgb > -7:   # virgo_w constraint
		      join = False
		      #print "There I am .. join? " , entity[0].id, entity[0].Vls, entity[0].sgb
...
def brent_modifications(G_list, galList):
  ...
  #addTo(G_list, galList, 14391, 14122)
  ...
  #formGroup(G_list, galList, [23324, 23521])
  ...
  #formGroup(G_list, galList, [35878, 37050, 40904, 43129])
  #formGroup(G_list, galList, [48332,49158,49452])
...
if __name__ == '__main__':
  ...
  if cluster == 'virgo':
    inFile  = 'AllSky.north.v18.cf3.csv'
  elif cluster == 'virgo_w':
    inFile  = 'AllSky.north.v18.cf3.csv'
  elif cluster == 'fornax':
    inFile  = 'AllSky.south.v18.cf3.csv'
  elif cluster == 'north':
    inFile  = 'AllSky.north.v18.cf3.csv'
  elif cluster == 'south':
    inFile  = 'AllSky.south.v18.cf3.csv'   
  ...
  ignore_list = [41618, 40001, 42447, 40240, 40851, 3085]   # April 18, 2016 (added pgc3085 SMC)
  Vh_zero = [29231,  31286,  40747,  46680, 166101, 166146, 166158, 166170, 166172, 166175, 166176]
  ...
  if   cluster == 'virgo' :
     galList = readgalList(table, R_max=R_max, ignore_list=ignore_list)   
  elif cluster == 'virgo_w' :
     galList = readgalList(table, R_max=R_max, skyPatch='vigo_w', ignore_list=ignore_list)  
  elif cluster == 'fornax': 
     galList =  readgalList(table, skyPatch='fornax', ignore_list=ignore_list)
  elif cluster == 'north' :
     galList = readgalList(table, ignore_list=ignore_list)
  elif cluster == 'south' :
     galList = readgalList(table, ignore_list=ignore_list)
  else:
     alpha = 336
     ...
     galList = readgalList(table, skyPatch='manual', sgl_min=alpha-step, sgl_max=alpha+step, sgb_min=delta-step, sgb_max=delta+step, ignore_list=ignore_list)    
  ...
  for iter in range(4):
	...
	## fixed
        ...
        inject_list=[40001, 40240, 40851, 42447]   
        for gal in galList:
...
####################################################################
	...
	for qp in range(0,2):
	  mergGroup(G_list, restrict=True)    
        ...
        if cluster == 'north':
          groupWrite('north.iter.'+str(int(iter))+'.v31.group', G_list, galList)
        elif cluster == 'south':
	  groupWrite('south.iter.'+str(int(iter))+'.v31.group', G_list, galList)
        elif cluster == 'manual':
	  groupWrite('manual.iter.'+str(int(iter))+'.v31.group', G_list, galList)
