...
def Mass(L_k):							def Mass(L_k):
  ...
  elif L > 1000.:					      |	  elif L > 0.0927 and L < 4.423:
    MtoL = 91.						      |	    #MtoL = 58.0*(L**-0.25)
  else:							      |	    return LtoM_func(L_k)
							      >	  elif L >= 4.423:
    MtoL = 32*(L**0.15)						    MtoL = 32*(L**0.15)
  								  
  							      <
  Mass_out = L_k * MtoL						  Mass_out = L_k * MtoL
...
class GalxyNode:						class GalxyNode:
  ...
  def __init__(self, id, gl, gb, sgl, sgb, Vls, Ks, Ty, dcf2,	  def __init__(self, id, gl, gb, sgl, sgb, Vls, Ks, Ty, dcf2,
    ...
    self.logK = m_logK(Ks, Vls, d=dcf2)				    self.logK = m_logK(Ks, Vls, d=dcf2)
...
def m_logK(m, Vls, d=0):					def m_logK(m, Vls, d=0):
    ...
    if Vls == 0 and d==0:					    if Vls == 0 and d==0:
      logK = 0							      logK = 0
	    ...
	    for i in range(len(G_list)): # group_indices[0]:		    for i in range(len(G_list)): # group_indices[0]:
	      if  G_list[i][0].nest != 41220:  # Virgo			      if  G_list[i][0].nest != 41220:  # Virgo
		    ...
		    if ang12 <= 2*r:						    if ang12 <= 2*r:
		      					      ...
		      					      |
		      					      |			      if ang12 <= 1.02*(180./pi)*atan(1.0*G_l
							      <
							      <
		      if ang12 <= 1.05*(180./pi)*atan(1.3*G_l <
			  d1 = G_list[i][0].mDist					  d1 = G_list[i][0].mDist
			  e1 = d1 * G_list[i][0].mDistErr				  e1 = d1 * G_list[i][0].mDistErr
			  ...
			  if ang12 > 1.05*(180./pi)*atan(1.0* |				  if ang12 > 1.02*(180./pi)*atan(1.0*
			    join = True							    join = True
			  if d1==0 or d2==0:						  if d1==0 or d2==0:
			    join = True							    join = True
			  if ang12 <= 1.05*(180./pi)*atan(1.0 |				  if ang12 <= 1.02*(180./pi)*atan(1.0
...
def brent_modifications(G_list, galList):			def brent_modifications(G_list, galList):
  ...
  manual_group(G_list, galList, [39225, 39023, 38881, 39145])	  manual_group(G_list, galList, [39225, 39023, 38881, 39145])
							      >	  
...
def manual_group(G_list, galList, id_list, verbose=False):  #	def manual_group(G_list, galList, id_list, verbose=False):  #
    ...
    if len(my_group)>0: 					    if len(my_group)>0: 
      root = LoosgalListJoint(my_group, grID = 900000000)	      root = LoosgalListJoint(my_group, grID = 900000000)
      new_Group = NodeLeaf(root)				      new_Group = NodeLeaf(root)
      G_list.append(new_Group)  				      G_list.append(new_Group)  
...
if __name__ == '__main__':					if __name__ == '__main__':
							      ...
							      |	  table = np.genfromtxt('M_L_curve.csv' , delimiter=',', fill
  ...
  if cluster == 'all' :						  if cluster == 'all' :
     ...
     alpha =  333.4714					      |	     alpha =  120
     delta = -7.0663					      |	     delta = 0
     step = 10						      |	     step = 15
...
#############################################################	#############################################################
	...
	for qp in range(0,2):						for qp in range(0,2):
	  mergGroup(G_list, restrict=True)    				  mergGroup(G_list, restrict=True)    
        ...
        groupWrite(cluster+'.iter.'+str(int(iter))+'.v37.grou |	        groupWrite(cluster+'.iter.'+str(int(iter))+'.v38.5.gr
        #sys.exit(1)						        #sys.exit(1)
