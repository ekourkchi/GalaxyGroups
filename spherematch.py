...
# 
# 
#   DESCRIPTION:	This python code imports spherematch_c package
#                       It needs to have spherematch_c.so in the same directory or installed properly in a library directory
#                      
#                       Added functions by Ehsan:
#                                                1) match_sdr_omp   --> finds histogram (good for <DR>)
#                                                2) match_sss_omp   --> finds histogram (good for <RR> and <DD>)
#                                                3) match_esn_omp   --> only find pairs
#                      The other functions were provided with developers of Astrometry.net package   
# ****************************************************************

# This file is part of libkd.
# Licensed under a 3-clause BSD style license - see LICENSE
#from astrometry.libkd import spherematch_c
import spherematch_c as spherematch_c    # This is the library that is provided in C
...
############# Match ESN OMP (including histogram calculations)
## This functions is able to get different S-arrays, when we corss correlate Data-Random files.
#  x1, x2 --> The coordintates of two sets of points to be matched. The size of x1 should be much smallet than x1 (Since the main kd-tree is produced here)
#  radius --> the maximum desired angular separation of the paurs
#  S --> S values associted to x1
#  S_rand --> S values associted to x2 (normally for data-random pairs, this is S-value of the random points)
#  low, min --> lower and upper bound of the calculated historgram
#  n_bins --> No. of bins of the histogram
#  We use default values for the other parameters
#
#  Output: histogram
#  Description: First, it finds all the pairs and then it calculated the histogram of the corresponding S-numbers of the pairs. 
#  S is physically connected to the angular separation of the pairs. It is easier to think of it as the actual physical distance between points.

def match_sdr_omp(x1, x2, radius, S, S_rand, low,  powmin, powmax, n_bins, n_size=0, notself=False, permuted=True):
  ...
  s_rand = S_rand.astype(np.float64)
  
  histogram = spherematch_c.match_sdr_omp(fx1, fx2, radius, s, s_rand, low,  powmin, powmax, n_bins, n_size, notself, permuted)
  
  
  return histogram

############# Match ESN OMP (including histogram calculations)
# This function is the same as previous one. But it assumes that both input data are the same, i.e. x1 is only a small part of x2 (only 100 points)
# Therefore the S-array is only passed onece. 

def match_sss_omp(x1, x2, radius, S, low,  powmin, powmax, n_bins, n_size=0, notself=False, permuted=True):
  ...
  histogram = spherematch_c.match_sss_omp(fx1, fx2, radius, s, low,  powmin, powmax, n_bins, n_size, notself, permuted)
...
############# Match ESN OMP
# The parameters are the same what we have in the previous function
# It does not calculate any histogram, and it only returns the discovered pair of points

def match_esn_omp(x1, x2, radius, n_size=0, notself=False, permuted=True):
    
    if n_size==0:
      n_size = len(x2)
      
    (fx1, fx2) = _cleaninputs(x1, x2)
    (inds,dists) = spherematch_c.match_esn_omp(fx1, fx2, radius, n_size, notself, permuted)
