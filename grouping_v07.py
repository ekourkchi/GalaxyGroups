...
def addGalGroup(G_list, galList):
    ...
    for i in indices[::-1]:
            ...
            d = G_list[i][0].Vls / H0
            if d < 1: d = 1 
            r = 2.*(180.*atan(G_list[i][0].R_2t2*1.05*sqrt(1.5)/d)/pi)
		...
		if Dist1 <= 1: Dist1 = 1.
	        ...
	        if Bol==True  and ang12 <= (180.*atan(1.1*G_list[i][0].R_2t2/(Dist1))/pi) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 3.0*sig_p:
  ...
  for iter in range(11):
	...
	for group in G_list:
	  ...
	  virgoPlot(G_list, galList, R_max=R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.iter.'+str(int(iter))+'.v21.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.iter.'+str(int(iter))+'.v21.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.iter.'+str(int(iter))+'.v21.group', G_list, galList)
        else:
	  groupWrite('south.iter.'+str(int(iter))+'.v21.group', G_list, galList)
	...
	for galaxy in galList: galaxy.inGroup = 0
	   ...
	   ## the best view and size happens in eps format
           plt.savefig('virgo.iter.'+str(int(iter))+'.v21.eps', dpi=600)
           print 'done with ...', 'virgo.iter.'+str(int(iter))+'.v21.eps'
        elif cluster == 'fornax':
	   plt.savefig('fornax.iter.'+str(int(iter))+'.v21.eps', dpi=600) 
	   print 'done with ...', 'fornax.iter.'+str(int(iter))+'.v21.eps'
