...
if __name__ == '__main__':
   ...
   EDD = np.genfromtxt('/home/ehsan/GalGroups/start/AllSky/2M++_allsky.csv' , delimiter=',', filling_values="-100000", names=True, dtype=None )
   EDD_pgc = EDD['pgc']  # EDD
   EDD_gl_ = EDD['SGL']
   EDD_gb_ = EDD['SGB'] 
   EDD_Vhel = EDD['Vhel'] 
   ...
   myTable.add_column(Column(data=empty,name='pgc_M++', dtype=np.dtype(int)))
   myTable.add_column(Column(data=empty,name='sgl_M++', format='%0.4f'))
   myTable.add_column(Column(data=empty,name='sgb_M++', format='%0.4f'))
   myTable.add_column(Column(data=empty,name='Vhel_M++', format='%0.1f'))
   ...
   myTable.write('M++_LEDA_match.txt', format='ascii.fixed_width',delimiter='|', bookend=False)
   print myTable
