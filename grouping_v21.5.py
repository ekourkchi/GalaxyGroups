	    ...
	    for i in range(len(G_list)): # group_indices[0]:
	      if  G_list[i][0].nest != 41220:  # Virgo
		    ...
		    if ang12 <= 2*r:
		      ...
		      if ang12 <= 1.02*(180./pi)*atan(1.0*G_list[i][0].R_2t2/d) and abs(entity[0].Vls - G_list[i][0].Vls) < 2.5*sig_p:
			  d1 = G_list[i][0].mDist
			  e1 = d1 * G_list[i][0].mDistErr
			  d2 = entity[0].dcf2
			  e2 = d2 * entity[0].ed
			  delt = abs(d1-d2)
			  join = False
			  if ang12 > 1.02*(180./pi)*atan(1.0*G_list[i][0].R_2t2/d) and d1!=0 and d2!=0 and delt<max(e1,e2):
			    join = True
			  if d1==0 or d2==0:
			    join = True
			  if ang12 <= 1.02*(180./pi)*atan(1.0*G_list[i][0].R_2t2/d):
			    join = True
...
####################################################################
	...
	for qp in range(0,2):
	  mergGroup(G_list, restrict=True)    
        ...
        groupWrite(cluster+'.iter.'+str(int(iter))+'.v38.5.group', G_list, galList)
        #sys.exit(1)
