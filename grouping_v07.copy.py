...
if __name__ == '__main__':
  ...
  if cluster == 'virgo':
    inFile  = 'AllSky.north.v11.csv'
  elif cluster == 'fornax':
    inFile  = 'AllSky.south.v11.csv'
  elif cluster == 'north':
    inFile  = 'AllSky.north.v11.csv'
  else:
    inFile  = 'AllSky.south.v11.csv'   
  ...
  #for ML in [60,61,62,63,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80]:
	...
	#galList.pop(0) 
	  ...
	  virgoPlot(table, G_list, R_min, R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.ML.'+str(int(ML))+'.v20.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.ML.'+str(int(ML))+'.v20.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.ML.'+str(int(ML))+'.v20.group', G_list, galList)
        else:
	  groupWrite('south.ML.'+str(int(ML))+'.v20.group', G_list, galList)
	...
	for galaxy in galList: galaxy.inGroup = 0
        ...
        if cluster == 'virgo':
           plt.savefig('virgo.ML.'+str(int(ML))+'.v20.eps', dpi=600)  # the best view and size happens in eps format
        elif cluster == 'fornax':
	   plt.savefig('fornax.ML.'+str(int(ML))+'.v20.eps', dpi=600)  # the best view and size happens in eps format
