...
class GalxyNode:
  ...
  def setMeanDist(self, Dist, errDist, GRP_vel = 0):
	...
	vel = self.Vls
      ...
      self.logK = m_logK(self.Ks, vel, Dist, 0)
      self.M_v2 = Mass(10**self.logK)
      self.R_2t2 = 0.215*((self.M_v2/1.E12)**(1./3))  # Mpc
...
def Fornax_readgalList(table, R_min, R_max, n_gal, useDCF2=True):
  ...
  while(i < N_galaxies and limit_counter<n_gal ):
    ...
    if  Vls[i] < 4000 and Ks[i] > 0 and sgl[i]<303 and sgl[i]>240 and sgb[i]>-57 and sgb[i]<-20:  # -45
...
def readgalList(table, R_min, R_max, n_gal, useDCF2=True):
  ...
  while(i < N_galaxies and limit_counter<n_gal):
    ...
    if Vls[i] < 4000 and Ks[i] > 0 and ang12 > R_min and ang12 < R_max: # and sgl[i]<113 and sgl[i]>79  and sgb[i]<8 and sgb[i]>-35  :
       #Ks[i] = Median_k
  ...
  return galList
################################################################  
def galListJoint(AllgalList, angle_limit = 10, grID = 100000000):
  
  #angle_limit = 20   # degree
  list_dis_problem=[]
  
  galList = []
  for gal in AllgalList:
    if gal.dcf2>0 and gal.Vls>0:
      galList.append(gal)
  
  print len(galList)
  print len(AllgalList)
...
def groupWrite(outfile, G_list, galList):
  ...
  for galaxy in galList:
      ...
      if galaxy.inGroup == 0. and ((galaxy.dcf2 != 0 and galaxy.dcf2<=10) or (galaxy.Vls <=1000)):
	flag = 0
	galaxy.flag = 0
        pgc = galaxy.id  
        gl = galaxy.gl  
...
def R_node(root, fraction):
  ...
  for i in range(1,N):
      ...
      logK[i-1]  = (m_logK(galList[i].Ks, galList[i].Vls, 0, galList[i].dcf2))
...
def fornaxPlot(G_list, galList):
    ...
    for i in range(0, NoGroups):  # for all groups
	  ...
	  #if G_list[i][0].Vls <= 2000:
	    #Blue = (2000.-G_list[i][0].Vls)/2001
	    #Green = 1. - Blue
	    #Red = 0
	  #elif G_list[i][0].Vls <= 5000:
	    #Green = (5000.-G_list[i][0].Vls)/3001
	    #Red = 1. - Green
	    #Blue = 0	  
	  #else: 
	  ...
	  for j in range(1, len(G_list[i])):
	      ...
	      RA0 = G_list[i][j].ra   #  point.fk5.ra.degree
	      DEC0 = G_list[i][j].dec   #  point.fk5.dec.degree
	      X.append(RA0)
	      Y.append(DEC0)
...
def virgoPlot(table, G_list, R_min, R_max):
  ...
  for i in range(len(id)):
    if (dcf2[i]>0 and dcf2[i]<10) or Vls[i]<1000:
      sgll.append(sgl[i])
      sgbb.append(sgb[i])
      Vlss.append(Vls[i])
...
#################################################################
def mergGroup(GG_list, restrict=False):
    
    
    #Lum_mass = []
    #for i in range(0, len(GG_list)):
        #Lum_mass.append(GG_list[i][0].M_v2)
    #Lum_mass = np.asarray(Lum_mass)
    #indices = np.argsort(Lum_mass)
    #G_list = []
    #for i in indices:
      #G_list.append(GG_list[i])
	...
	if G_list[i][0].Vls < 75 : 
	  r1 = (180.*atan(G_list[i][0].R_2t2/(75./H0))/pi)
	if G_list[j][0].Vls < 75 :
	  r2 = (180.*atan(G_list[j][0].R_2t2/(75./H0))/pi)
...
#######################################################################
	  ...
	  ##for override in [14077]:
	  #if (idd1==13418 and  idd2==14077) or (idd1==14077 and  idd2==13418):
	      #Bol = True
...
#######################################################################
	...
	if ang12 <= 1.1*max(r1,r2) and max(r1,r2)<6:
	  if d1!=0 and d2!=0 and delt <= 2.0*(e1+e2) and abs(v1-v2) <= max(2*Sigquad,3.0*max(sig1,sig2)):
	      Bol = True
	  elif (d1==0 or d2==0) and abs(v1-v2) <= max(2*Sigquad,3.0*max(sig1,sig2)):
	      Bol = True
	...
	if ang12 <= 1.1*(max(r1,r2)-min(r1,r2)) and max(r1,r2)<6:
	  if d1!=0 and d2!=0 and delt <= 2.0*(e1+e2) and abs(v1-v2) <= max(4*Sigquad,4.0*max(sig1,sig2)):
	      Bol = True
	  elif (d1==0 or d2==0) and abs(v1-v2) <= max(4*Sigquad,4.0*max(sig1,sig2)):
	      Bol = True	      
#######################################################################
	...
	#if ang12 <= 0.9*(r1+r2) and max(r1,r2)<6:
	  #if d1!=0 and d2!=0 and delt <= (e1+e2) and abs(v1-v2) <= 1.*max(sig1,sig2):
	      #Bol = True
	  #elif (d1==0 or d2==0) and abs(v1-v2) <= 1.*max(sig1,sig2):
	      #Bol = True
	...
	#if ang12 <= 1.05*(r1+r2) and max(r1,r2)<6:
	  #if d1!=0 and d2!=0 and delt <= 1.0*min(e1,e2) and abs(v1-v2) <= 0.5*max(sig1,sig2):
	      #Bol = True
	  #elif (d1==0 or d2==0) and abs(v1-v2) <= 0.5*min(sig1,sig2):
	      #Bol = True
...
def addGalGroup(G_list, galList):
    ...
    modified_G_list = []
    for group in G_list:
      #if group[0].R_theta*180./pi > 10. or group[0].mDist > 8:
      if group[0].R_theta*180./pi > 10. or group[0].R_theta*180./pi < 0.2:
    ...
    for i in indices[::-1]:
            ...
            p=0
            if G_list[i][0].Vls < 75: 
	       r = 2.*(180.*atan(G_list[i][0].R_2t2*1.05*sqrt(1.5)/(75./H0))/pi)
	    else:
		...
		if Dist1 == 0:
		  if G_list[i][0].Vls<75: 
		    ...
		    Dist1 = G_list[i][0].Vls/H0
		if Bol==True and subgalList[p].dcf2==0 and Dist1 !=0 and ang12 <= (180.*atan(1.1*G_list[i][0].R_2t2/(Dist1))/pi) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 3.0*sig_p:
		  ...
		  subgalList.pop(p)
		  p-=1
	        elif Bol==True  and ang12 <= (180.*atan(1.1*G_list[i][0].R_2t2/(Dist1))/pi) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 3.0*sig_p:
...
#################################################################
def IndividualGal(G_list, galList_org):
  ...
  while p < N-1:
    ...
    while q!=pivot:
      ...
      if grp.mDist == 0: 
	thet = (180.*atan(grp.R_2t2/(grp.Vls/H0))/pi)
	if grp.Vls<75:
	  thet = (180.*atan(grp.R_2t2/(75./H0))/pi)
      else: 
	thet = (180.*atan(grp.R_2t2/(grp.mDist))/pi)
      if ang12 <= 1.0*thet:
	...
	if eDist0*eDist1 != 0 and Dist0*Dist1 != 0:
	    ...
	    sanityDist = 0.
	if (abs(grp.Vls-individuals[q].Vls) <= 2.0*sig_p) or (sanityDist > 0. and sanityDist<=2.0 and abs(grp.Vls-individuals[q].Vls) <= 2.0*sig_p):
	#if (abs(grp.Vls-individuals[q].Vls) <= 3.0*sig_p):
...
def LoosgalListJoint(galList, grID = 500000000):
   ...
   if meanDist != 0:
     ...
     for gal in galList:
       if gal.dcf2 == 0 or gal.dcf2 != 0:
	 gal.logK = m_logK(gal.Ks, gal.Vls, G_dist, 0)
	 gal.mDist = G_dist
...
if __name__ == '__main__':
  ...
  if cluster == 'virgo':
    inFile  = 'AllSky.north.v10.csv'
  elif cluster == 'fornax':
    inFile  = 'AllSky.south.v10.csv'
  elif cluster == 'north':
    inFile  = 'AllSky.north.v10.csv'
  else:
    inFile  = 'AllSky.south.v10.csv'   
  ...
  #ML = 60
  #if cluster == 'virgo':
    #root = readTree('virgo.ML.'+str(int(ML))+'.v16.tree')
  #elif cluster == 'fornax':
    #root = readTree('fornax.ML.'+str(int(ML))+'.v16.tree')
  #elif cluster == 'north':
    #root   = readTree('north.ML.'+str(int(ML))+'.v16.tree')
  #else:
    #root = readTree('south.ML.'+str(int(ML))+'.v16.tree')
  #print "List making ..."
  #galList = NodeLeaf(root)
  ...
  #for ML in [60,61,62,63,64,65,66,67,68,69,70]:
  for ML in [60,61,62,63,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80]:
	...
	###if cluster == 'virgo':
	  ...
	  ###ehsan.write('virgo_dist_error.ML.'+str(int(ML))+'.v16.txt', format='ascii',delimiter=',')
	###elif cluster == 'fornax':
	  ###ehsan = Table()
	  ###ehsan.add_column(Column(data=badDist,name='pgc'))
	  ###ehsan.write('fornax_dist_error.ML.'+str(int(ML))+'.v16.txt', format='ascii',delimiter=',')
	  
	
	#if cluster == 'virgo':
	  #writeTree('virgo.ML.'+str(int(ML))+'.v16.tree', root) # write the tree
	#elif cluster == 'fornax':
	  #writeTree('fornax.ML.'+str(int(ML))+'.v16.tree', root) # write the tree
        #elif cluster == 'north':
          #writeTree('north.ML.'+str(int(ML))+'.v16.tree', root) # write the tree
        #else:
	  #writeTree('south.ML.'+str(int(ML))+'.v16.tree', root) # write the tree
	  ...
	  #root = readTree('fornax.ML.'+str(int(ML))+'.v16.tree')
        #else:
	  #root   = readTree('north.ML.'+str(int(ML))+'.v16.tree')
	  #root_1 = readTree('south.ML.'+str(int(ML))+'.v16.tree')
	...
	for pq in range(0,10): 
	  G_list += IndividualGal(G_list, galList)
	  for qp in range(0,20):
	    mergGroup(G_list)
	...
	#G_list_1 = groupID(root_1, ML)
        #for pq in range(0,10): 
	  #G_list_1 = addGalGroup(G_list_1, root_1)
        #for pq in range(0,10): 
	  #G_list_1 = addGalGroup(G_list_1, root_1)	
	  #mergGroup(G_list_1)
	  #G_list_1 = addGalGroup(G_list_1, root_1)	
        #IndivG_lis_1 = IndividualGal(G_list_1, root_1)
        #G_list_1 = G_list_1 + IndivG_lis_1  
        #mergGroup(G_list_1, restrict=True)
	...
	#galList.pop(0) 
	  ...
	  virgoPlot(table, G_list, R_min, R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.ML.'+str(int(ML))+'.v16.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.ML.'+str(int(ML))+'.v16.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.ML.'+str(int(ML))+'.v16.group', G_list, galList)
        else:
	  groupWrite('south.ML.'+str(int(ML))+'.v16.group', G_list, galList)
	...
	for galaxy in galList: galaxy.inGroup = 0
        ...
        if cluster == 'virgo':
           plt.savefig('virgo.ML.'+str(int(ML))+'.v16.eps', dpi=600)  # the best view and size happens in eps format
        elif cluster == 'fornax':
	   plt.savefig('fornax.ML.'+str(int(ML))+'.v16.eps', dpi=600)  # the best view and size happens in eps format
