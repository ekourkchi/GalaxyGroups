...
#########################################
     ...
     if pgc[i] in [2017209, 4713565, 5056949, 5056950, 5057224, 5064293, 86577, 907446]:
       flag[i] += 1
...
# </> ################# Finished Base on new.170226


     if pgc[i] in [4713558,4713561,4713552,4713556,4713554,4713555,4713563,45303,2807140,5056946]:
       flag[i] += 1

################################################################################# 
# file; "target_updates_2016-11-9"
     if pgc[i] == 5056932:
       DM[i]   = 27.81
       eDM[i]  = 0.1
       Dist[i] = 10**(DM[i]/5.-5.)
     ...
     # Taken from this file: new_nearby_galaxies.20170220
     if pgc[i] == 5056944:
       Vhelio[i] =-38
       Vls[i] = Vh2Vls(gl[i], gb[i], Vhelio[i])
       Vls_source[i] = 'Mod. Brent'  
...
#################################################################################  
## The following sources are taken manually from the file: new_nearby_galaxies.20170220
#################################################################################  
#################################################################################  
      if True:
	pgc_new = 5098252
	...
	Vls_new = 87
	Vhelio_new = 376
	Vls_source_new = 'Brent'
	Dist_new = 1.29
	eD_new = 0.15
	DM_new   = (log10(Dist_new)+5.)*5.
        eDM_new  = (5.*eD_new)/log(10.)   
	OBJECT_new = 'Antlia-B'
	BB_new = -100000
	myTable.add_row([pgc_new, ra_new, dec_new, gl_new, gb_new, sgl_new, sgb_new, coordinate_new, \
	  Ty_new, Ty_source_new, BB_new, Ks_new, Ks_source_new, Vls_new, Vhelio_new, Vls_source_new, Dist_new, eD_new, DM_new, eDM_new, OBJECT_new])
...
################################################################################# 
      ...
      myTable.write('AllSky.v26.cf3.csv', format='ascii.fixed_width',delimiter=',', bookend=False)
