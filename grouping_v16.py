...
def readgalList(table, skyPatch='none', sgl_min=0, sgl_max=0, sgb_min=0, sgb_max=0, Vls_max=4000, ignore_list=[]):
  ...
  for i in range(N_galaxies):
    ...
    if  skyPatch=='manual':
      if Vls[i] < Vls_max  and sgl[i]<sgl_max and sgl[i]>sgl_min and sgb[i]>sgb_min and sgb[i]<sgb_max:
        Bool = True
    elif skyPatch=='local':
      if Vls[i] < Vls_max:
	Bool = True
...
#################################################################
def manual_group(G_list, galList, id_list):  # my_group: id list of members
...
if __name__ == '__main__':
  ...
  Virgo_foreback = [40045, 42081, 43072, 44491, 43601, 42089, 43254, 42833]
...
####################################################################
	...
	for qp in range(0,2):
	  mergGroup(G_list, restrict=True)    
	    ...
	    G_list += IndividualGal(g_lst)
	    meanDist    = G_list[0][0].mDist
	    meanDistErr = G_list[0][0].mDistErr
	    for galaxy in g_lst:
	      if galaxy.inGroup == 0: 
		galaxy.inGroup=-1
	    ...
	    manual_group(G_list, galList, id_Virgo_w)
        ...
        groupWrite(cluster+'.iter.'+str(int(iter))+'.v33.group', G_list, galList)
