...
# **************************************
def readgrouplist(filename, skyPatch='none', sgl_min=0, sgl_max=0, sgb_min=0, sgb_max=0):
...
def force_add(SuperList, GroupList, SGID=None , obj_id=None):
  ...
  #print 'flag: ', obj.flag
  remove_obj(SuperList, GroupList, obj_id)
...
#################################################################
def remove_obj(SuperList, GroupList, id):  # removing obj from SG-list
...
def populate(heads, GroupList, except_id=None):
  ...
  if except_id != None:
    while i < len(heads):
     objlist = heads[i][1]
     for objects in objlist:
       if objects.id == except_id:
          #print except_id, heads[i][0].id
          except_id = heads[i][0].id
          i = 1000000000 # break while
          break # break for
     i+=1
  ...
  for grp in GroupList:
	...
	for i in range(len(heads)):
	  ...
	  if d != None and (except_id==None or heads[i][0].id != except_id):
	    indx.append(i)
	    dist.append(d)
...
if __name__ == '__main__':
   ...
   filename = "north.iter.2.v32.group" 
   print filename
   ...
   trimGroup_vel(SuperList, 300013418, Vmin=1000)
   SuperList = populate(SuperList, GroupList, except_id=300013418)   
   # except_id is the ID of one of the SGroups members

   force_add(SuperList, GroupList, SGID=300014391 , obj_id=300014163) 
   force_add(SuperList, GroupList, SGID=300014391 , obj_id=300014169) 
   force_add(SuperList, GroupList, SGID=300014391 , obj_id=300013090) 
   
   ####remove_obj(SuperList, GroupList, 300014391)
   
   force_add(SuperList, GroupList, SGID=700013505 , obj_id=300013620) 
   
   
   forceMerge(SuperList, 300016779, 4078612)
   
   remove_obj(SuperList, GroupList, 200070090)
   remove_obj(SuperList, GroupList, 300070676)
   remove_obj(SuperList, GroupList, 300006172)
   
   remove_obj(SuperList, GroupList, 300008631)
   SuperList = populate(SuperList, GroupList, except_id=10300010266)  
   
  
   print "\n total super groups #:", len(SuperList)
   
   
   SGroupwrite('north.iter.2.v32.supergroup', SuperList, GroupList)
