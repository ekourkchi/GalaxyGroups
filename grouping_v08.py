...
def virgoPlot(G_list, galList, R_min=6.0, R_max=100000000.):
...
#################################################################
def IndividualGal(G_list, galList_org, coeff=1.0):
...
if __name__ == '__main__':
  ...
  if cluster == 'virgo':
    inFile  = 'AllSky.north.v12.csv'
  elif cluster == 'fornax':
    inFile  = 'AllSky.south.v12.csv'
  elif cluster == 'north':
    inFile  = 'AllSky.north.v12.csv'
  elif cluster == 'south':
    inFile  = 'AllSky.south.v12.csv'   
...
##########################################################################################
        ...
        #G_list_heuristic= IndividualGal(G_list, galList, coeff=1.5)
        #group_moderator(G_list_heuristic)
        #i = 0
        #NoGroups = len(G_list_heuristic)
        #while i < NoGroups:
          #group = G_list_heuristic[i]
	  #head = group[0]
	  #galaxies = group[1:]
	  #angular_radii =  Theta_gr(head, galaxies)
          #Dist = head.mDist
          ##Dist = head.Vls/H0
          ##if Dist < 1: Dist = 1
          #Rg_obs = angular_radii*Dist
          #if  Dist==0 or Rg_obs > 0.1*head.R_2t2 or angular_radii*180./pi > 10.:
	    #for galaxy in galaxies:
	        #galaxy.inGroup = 0
	        #galaxy.flag = 0
	    
	    #G_list_heuristic.pop(i) 
	    #NoGroups-=1
	    #i-=1
          #i+=1
##########################################################################################
        #G_list+= G_list_heuristic
#########################################################################################
	...
	for galaxy in galList:
	  ...
	  virgoPlot(G_list, galList, R_max=R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.iter.'+str(int(iter))+'.v22.group', G_list, galList)
        elif cluster == 'fornax': 
	  groupWrite('fornax.iter.'+str(int(iter))+'.v22.group', G_list, galList)
        elif cluster == 'north':
          groupWrite('north.iter.'+str(int(iter))+'.v22.group', G_list, galList)
        else:
	  groupWrite('south.iter.'+str(int(iter))+'.v22.group', G_list, galList)
	...
	for galaxy in galList: galaxy.inGroup = 0
	   ...
	   ## the best view and size happens in eps format
           plt.savefig('virgo.iter.'+str(int(iter))+'.v22.eps', dpi=600)
           print 'done with ...', 'virgo.iter.'+str(int(iter))+'.v22.eps'
        elif cluster == 'fornax':
	   plt.savefig('fornax.iter.'+str(int(iter))+'.v22.eps', dpi=600) 
	   print 'done with ...', 'fornax.iter.'+str(int(iter))+'.v22.eps'
