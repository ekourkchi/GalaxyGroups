    ...
    def onpick(self, event):
        ...
        if event.mouseevent.button == 1:   # It's just active at left mouse click
            ...
            if N > 1:
                ...
                myTable.add_column(
                    Column(data=self.dcf2_gal[ind],  name='dcf2'))
                myTable.add_column(Column(data=self.ed_gal[ind],  name='ed'))
                myTable.add_column(
                    Column(data=self.mDist_gal[ind],  name='mDist'))
                myTable.add_column(
    ...
    def single_info(self, dataind):
        ...
        if self.mode == 'cartesian':
            sgx, sgy, sgz = xyz_list(self.sgl_gal[ind], self.sgb_gal[ind], self.Vls_gal[ind],
                                     self.dcf2_gal[ind], self.mDist_gal[ind], self.flag_gal[ind])
...
def file_deliver(filee):
    ...
    if Have_north and not Have_south:
        ...
        dcf2 = np.concatenate(
            (north_table['dcf2'][north_ind], south_table['dcf2'][south_ind]))
        ed = np.concatenate(
            (north_table['ed'][north_ind], south_table['ed'][south_ind]))
        objname = np.concatenate(
            (north_table['objname'][north_ind], south_table['objname'][south_ind]))
        ...
        nest1 = np.concatenate(
            (north_table['nest'][north_ind], south_table['nest'][south_ind]))
        dcf21 = np.concatenate(
            (north_table['dcf2'][north_ind], south_table['dcf2'][south_ind]))
        ed1 = np.concatenate(
            (north_table['ed'][north_ind], south_table['ed'][south_ind]))
        objname1 = np.concatenate(
            (north_table['objname'][north_ind], south_table['objname'][south_ind]))
...
""")
    ...
    parser.add_option("-G", action="store_true",
                      help="Plot Virgo Border", default=False)
...
######################################################
    if virgo_on:
        ...
        if mode == "supergalactic":
            xA, yB = transform_list(parent.alpha, parent.delta, [
                                    110, 105, 105, 96, 0], [-3, -3, -4, -4, 0])
            line, = plt.plot(xA[0:2], yB[0:2], '-',
                             markersize=2, color='black', picker=False)
            line, = plt.plot(xA[1:3], yB[1:3], '-',
                             markersize=2, color='black', picker=False)
            line, = plt.plot(xA[2:4], yB[2:4], '-',
                             markersize=2, color='black', picker=False)
...
######################################################
    ...
    while gr == 2:
        ...
        point, = plt.plot(X, Y, 'o', markersize=3,
                          color=colour, markeredgecolor=colour)
        parent.list_point.append(
            [point, (Red, Green, Blue), colour, Vls_grp, True])
...
class Plot_page(tk.Frame):
    ...
    def __init__(self, parent, controller, filee, mode, alpha, delta, width, pgc, Vmin, Vmax, SGX, SGY, SGZ, Projection, xt, yt, zt):
        ...
        info_bts = Button(resetax, '', color='lightgray',
    ...
    def show_plot(self):
        ...
        self.line, = plt.plot(A, B, '.', markersize=1, color='white',
                              markeredgecolor='white', picker=5)   # color='#696969'

        for i in range(len(id_gal)):
            l, = plt.plot([A[i]], [B[i]], 'o', markersize=2.5,
                          color='white', markeredgecolor=color_table(Vls_gal[i]))
            self.lline.append([l, color_table(Vls_gal[i]),
                               Vls_gal[i], True, '#696969'])
        #########
        ...
        ClusterPlot(id, A_cluster, B_cluster, Vls, R_2t2,
                    mDist, nest, flag, self, mode=mode)
        ...
        annotate('Random Colors ?', (0.86, 0.21),
                 xycoords='figure fraction', size=10, color='black')
