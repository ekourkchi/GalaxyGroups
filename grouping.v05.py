...
def mergGroup(G_list, restrict=False):
	...
	if G_list[i][0].mDist != 0 and G_list[j][0].mDist != 0 :
	  r1 = (180.*atan(1.11*G_list[i][0].R_2t2/(G_list[i][0].mDist))/pi)
	  r2 = (180.*atan(1.11*G_list[j][0].R_2t2/(G_list[j][0].mDist))/pi)
	else: 
	  r1 = (180.*atan(1.11*G_list[i][0].R_2t2/(G_list[i][0].Vls/H0))/pi)
	  r2 = (180.*atan(1.11*G_list[j][0].R_2t2/(G_list[j][0].Vls/H0))/pi)
	...
	# using dynamical velocity dispersions calculated based on the luminosities
        sig1 = (1.33*G_list[i][0].M_v2 / (2.0E6))**(1./3)
	sig2 = (1.33*G_list[j][0].M_v2 / (2.0E6))**(1./3)
	...
	for override in [13753, 13756, 13250, 13281]:
	  if idd1==override or idd2==override:
	    Bol = True
...
def addGalGroup(G_list, root):
    ...
    for i in indices[::-1]:
            ChangeGroup = False
            p=0
	    r = 2.*(180.*atan(1.11*G_list[i][0].R_2t2*1.05*sqrt(1.5)/(G_list[i][0].Vls/H0))/pi)
		...
		if  subgalList[p].inGroup == 1:
	        ...
	        sig_p = (1.33*G_list[i][0].M_v2 / (2.0E6))**(1./3)
		...
		if eDist0*eDist1 != 0 and Dist0*Dist1 != 0:
		    ...
		    sanityDist = 0.
		if Bol==True and subgalList[p].dcf2==0 and Dist1 !=0 and ang12 <= 1.0*(180.*atan(1.11*G_list[i][0].R_2t2/(Dist1-eDist1))/pi) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 2.*sig_p:
		  ...
		  subgalList.pop(p)
		  p-=1
	        elif Bol==True and subgalList[p].dcf2==0 and Dist1 ==0  and ang12 <= 1.0*(180.*atan(1.11*G_list[i][0].R_2t2/(G_list[i][0].Vls/H0))/pi) and abs(subgalList[p].Vls - G_list[i][0].Vls) < 2.*sig_p:
		  ...
		  p-=1
	        ...
	        elif Bol==True and sanityDist > 0. and sanityDist<=1 and ang12 <= 1.0*(180.*atan(1.11*G_list[i][0].R_2t2/(Dist1-eDist1))/pi):
...
if __name__ == '__main__':
  ...
  #ML = 60
  #if cluster == 'virgo':
    #root = readTree('virgo.ML.'+str(int(ML))+'.v14.tree')
  #elif cluster == 'fornax':
    #root = readTree('fornax.ML.'+str(int(ML))+'.v14.tree')
  #elif cluster == 'north':
    #root   = readTree('north.ML.'+str(int(ML))+'.v14.tree')
  #else:
    #root = readTree('south.ML.'+str(int(ML))+'.v14.tree')
  #print "List making ..."
  #galList = NodeLeaf(root)
  ...
  #for ML in [60,61,62,63,64]:
	...
	###if cluster == 'virgo':
	  ...
	  ###ehsan.write('virgo_dist_error.ML.'+str(int(ML))+'.v14.txt', format='ascii',delimiter=',')
	###elif cluster == 'fornax':
	  ###ehsan = Table()
	  ###ehsan.add_column(Column(data=badDist,name='pgc'))
	  ###ehsan.write('fornax_dist_error.ML.'+str(int(ML))+'.v14.txt', format='ascii',delimiter=',')
	  
	
	if cluster == 'virgo':
	  writeTree('virgo.ML.'+str(int(ML))+'.v14.tree', root) # write the tree
	elif cluster == 'fornax':
	  writeTree('fornax.ML.'+str(int(ML))+'.v14.tree', root) # write the tree
        elif cluster == 'north':
          writeTree('north.ML.'+str(int(ML))+'.v14.tree', root) # write the tree
        else:
	  writeTree('south.ML.'+str(int(ML))+'.v14.tree', root) # write the tree
	  ...
	  #root = readTree('fornax.ML.'+str(int(ML))+'.v14.tree')
        #else:
	  #root   = readTree('north.ML.'+str(int(ML))+'.v14.tree')
	  #root_1 = readTree('south.ML.'+str(int(ML))+'.v14.tree')
	...
	galList.pop(0) 
	  ...
	  virgoPlot(table, G_list, R_min, R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.ML.'+str(int(ML))+'.v14.group', G_list, root)
        elif cluster == 'fornax': 
	  groupWrite('fornax.ML.'+str(int(ML))+'.v14.group', G_list, root)
        elif cluster == 'north':
          groupWrite('north.ML.'+str(int(ML))+'.v14.group', G_list, root)
        else:
	  groupWrite('south.ML.'+str(int(ML))+'.v14.group', G_list, root)
	...
	for galaxy in galList: galaxy.inGroup = 0
        ...
        if cluster == 'virgo':
           plt.savefig('virgo.ML.'+str(int(ML))+'.v14.eps', dpi=600)  # the best view and size happens in eps format
        elif cluster == 'fornax':
	   plt.savefig('fornax.ML.'+str(int(ML))+'.v14.eps', dpi=600)  # the best view and size happens in eps format
