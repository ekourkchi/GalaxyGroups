  ...
  for i in range(0, NoGroups):  # for all groups
    ...
    if G_list[i][0].Vls <= 3500:
       ...
       for gal in G_list[i][1:]:
          sgl_lst.append(gal.sgl)
          sgb_lst.append(gal.sgb)
          v_galaxies.append(gal.Vls)
       
       
       mean = st.biweight_location(v_galaxies)
       Rg_bi = biweight_Rg(sgl_lst,sgb_lst, mean, dist)
...
####################################################################
	...
	for qp in range(0,2):
	  mergGroup(G_list, restrict=True)    
        ...
        groupWrite(cluster+'.iter.'+str(int(iter))+'.v44_test.group', G_list, galList)
        #sys.exit(1)
