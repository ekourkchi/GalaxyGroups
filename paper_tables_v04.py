...
def create_tables():
  ...
  for i in range(len(mDistErr_super_t)):
      if mDistErr_super_t[i] < 0.03: 
          mDistErr_super_t[i] = 0.03
...
########################################################################## 
  ...
  for i in range(len(mDistErr_grp)): 
      if mDistErr_grp[i] <0.03:
          mDistErr_grp[i] = 0.03
  ...
  myTable.add_column(Column(data=gl_gal, name='GLong', format='%0.4f'))
  ...
  myTable.add_column(Column(data=B_mag_gal, name='B_t', format='%0.2f'))
  myTable.add_column(Column(data=Ks_gal, name='K_t', format='%0.2f'))
  myTable.add_column(Column(data=logK_gal, name='lgL_K', format='%0.2f'))
  myTable.add_column(Column(data=Vhelio_gal, name='Vhel', format='%0.0f'))
  myTable.add_column(Column(data=Vls_gal, name='Vls', format='%0.0f'))
  myTable.add_column(Column(data=dcf2_gal, name='D_i', format='%0.2f'))
  myTable.add_column(Column(data=ed_gal*100, name='eDi', format='%0.0f'))
  ...
  #SGX = np.zeros(len(pgc_gal))
  #SGY = np.zeros(len(pgc_gal))
  #SGZ = np.zeros(len(pgc_gal))
  #for i in range(len(pgc_gal)):


      #if np.isnan(dcf2_gal[i]):
          #dist = Vls_gal[i]/75.
      #else: dist = dcf2_gal[i]
      
      #if pgc_gal[i] == 12911:
          #dist = 1744*18.09/1360  
      
      #SGX[i], SGY[i], SGZ[i] = XYZ(sgl_gal[i], sgb_gal[i], dist)
  ...
  myTable.add_column(Column(data=gl_galgrp, name='gGLong', format='%0.4f'))
  myTable.add_column(Column(data=gb_galgrp, name='gGLat', format='%0.4f'))  
  myTable.add_column(Column(data=sgl_galgrp, name='gSGL', format='%0.4f'))
  myTable.add_column(Column(data=sgb_galgrp, name='gSGB', format='%0.4f'))
  ...
  myTable.add_column(Column(data=Vhelio_galgrp, name='gVh', format='%0.0f'))
  ...
  myTable.add_column(Column(data=mDistErr_galgrp*100, name='geD', format='%0.0f'))
  ...
  #myTable.add_column(Column(data=SGX, name='SGX', format='%0.2f'))
  #myTable.add_column(Column(data=SGY, name='SGY', format='%0.2f'))
  #myTable.add_column(Column(data=SGZ, name='SGZ', format='%0.2f'))
...
#########################################################################
  ...
  intable = np.genfromtxt('AllSky.v26.cf3.csv' , delimiter=',', filling_values=None, names=True, dtype=None)  
  pgc  = intable['pgc']
  Vls  = intable['Vls']
  dcf2 = intable['dcf2'] 
  
  ind = np.where(Vls<4000)
  Vls_sel  = Vls[ind]
  dcf2_sel = dcf2[ind]
  
  
  ind = np.where(dcf2_sel>0)
  dcf2_sel_dcf3 = dcf2_sel[ind]
  print ''
  print '# of gal < 4000 : ', len(Vls_sel)
  print '# of gal < 4000 with dfc3: ', len(dcf2_sel_dcf3)
  

#########################################################################  

  intable = np.genfromtxt('all.iter.2.v44.group' , delimiter='|', filling_values=None, names=True, dtype=None)  
  Vls  = intable['Vls']
  dcf2 = intable['dcf2'] 
  ed = intable['ed'] 
  nest = intable['nest'] 
  ...
  print np.mean(Vls), np.std(Vls)/sqrt(len(flag))
  
  sum = 0
  denom = 0 
  for i in range(len(flag)):
      sum += dcf2[i]/(ed[i]*dcf2[i])**2
      denom += 1./(ed[i]*dcf2[i])**2
  
  print sum/denom, 1./sqrt(denom)
