...
def galJoint(galNode1, galNode2, ID):
   ...
   #newNode.sigma = sqrt((n1+n2)/(n1+n2-1.)) * sqrt(newNode.v2_av - newNode.v_av**2)    
   ...
   Rgroup = Rgr_max(newNode)
   #Rgroup = max(dist1 + galNode1.Rgroup, dist2 + galNode2.Rgroup)   # it's like Rgr_max
   #Rgroup = dist
   
   newNode.Rgroup = Rgroup
   
   
   if Rgroup !=0:
      newNode.Rho_gr = L_tot / ((4./3)*pi*Rgroup**3)  # group Density   
   else:
      newNode.Rho_gr = 0
   
   
   
   #theta = angle(l1, b1, l2, b2)*180/pi
   #theta1 = angle(l, b, l1, b1)*180/pi
   #theta2 = angle(l, b, l2, b2)*180/pi
   #R_theta = max(theta1 + galNode1.R_theta, theta2 + galNode2.R_theta)
   #newNode.R_theta = R_theta
   ...
   if n1+n2 < 200:
      ...
      newNode.r_g = r_g(newNode)
      M_v = (1.E9 * (2.5/G) * (Mpc_km*newNode.r_g) * sig**2)/M_sun  # solar mass
      
      #M_v = 2.0E6 * (sig**3)
      
      M12 = M_v / 1.E12
      R_2t = 0.215*(M12**(1./3))  # Mpc
...
class GalxyNode:
  ...
  def toString(self):
    ...
    print "Ty: ", self.Ty
    print "Rgroup: ",  self.Rgroup
    print "R_theta: ",  self.R_theta
    print "Rho_gr: ",  "%.4g" % self.Rho_gr
...
def groupIDCore(root, list, ML):
  ...
  param2 = (10**root.logK) / ((4./3)*pi*((root.R_2t2*1.05*sqrt(1.5))**3)) 
  param = min(param1,param2)
  
  #limit = Rho_cr(10**root.logK, ML)
  
  sigma_p = (root.M_v2 / (2.0E6))**(1./3)
  #r_g = root.M_v2*M_sun*(G/2.5)/(Mpc_km*1.E9*sigma_p*sigma_p)
  r_g = root.R_2t2*1.05*sqrt(1.5)
  delta = abs(root.Rgroup - r_g) / r_g  
  
  t_dynamical =  (r_g*Mpc_km) / (sqrt(2.5)*sigma_p)
  #delta = abs(t0_age - t_dynamical) / t0_age

  
  
  #delta = abs(root.Rgroup - (root.R_2t2*1.05*sqrt(3./2))) / (root.R_2t2*1.05*sqrt(3./2))
  #r_lim = 180.*atan(root.R_2t2/(root.Vls/H0))/pi
  #r_thet = root.R_theta
  #delta = abs(r_thet - r_lim) / r_lim
  ...
  if ML <= 43: MtoL = ML
  Tx = (pi/2.) * Mpc_km / sqrt(2.5) / (sigma_p/root.R_2t2)
  Rho_X = (Mpc_km*1000)**3/(4*pi*G*Tx**2)/M_sun/MtoL
  Rho_cr = (Mpc_km*1000)**3/(4*pi*G*t0_age**2)/M_sun/MtoL
  Rho_cr_universe = 1.5 * Rho_cr
  limit = Rho_X
  ...
  delta = abs(Rg_obs-root.R_2t2)/root.R_2t2
  
  #if delta < 0.05:
  if Rg_obs <= root.R_2t2 and angular_radii*180./pi < 10.:
  #if Rg_obs <= root.R_2t2*1.05*sqrt(1.5):
...
def treeAppend(myTable, root):
	...
	coordinate_src, Ty_src, Ks_src, Vls_src, objname, \
	  Rgroup, Rho_gr, R_theta, sigma, v_av, v2_av, nest, r_g, M_v, R_2t, M_v2, R_2t2, leftID, rightID])
...
def writeTree (filename, root):
    ...
    myTable.add_column(Column(data=empty,name='Rgroup'))
    myTable.add_column(Column(data=empty,name='Rho_gr'))
    ...
    myTable.add_column(Column(data=empty,name='r_g'))
    myTable.add_column(Column(data=empty,name='M_v'))
    myTable.add_column(Column(data=empty,name='R_2t'))
    ...
    #ascii.write(table, filename, \
       #names = ['pgc','sgl','sgb','Vls','logK','Ty','No_Galaxies','level','Rgroup','Rho_gr','R_theta', \
	 #'sigma','v_av','v2_av','nest','leftID','rightID','dcf2','ed'], \
       #delimiter=',')
...
def makeNode(table, index):
    ...
    newNode.Rgroup = table['Rgroup'][index]
    newNode.Rho_gr = table['Rho_gr'][index]
    ...
    newNode.r_g = table['r_g'][index]
    newNode.M_v = table['M_v'][index]
    newNode.R_2t = table['R_2t'][index]
...
def addGalaxyTable(galaxy, myTable, G_members, subGalaxies):
    ...
    level = galaxy.level  
    Rgroup = galaxy.Rgroup  
    Rho_gr = galaxy.Rho_gr  
    ...
    myTable.add_row([pgc, gl, gb, sgl, sgb, Vls, logK, Ty, dcf2, ed, mDist, mDistErr, G_members, subGalaxies, level \
			 , Rgroup, Rho_gr, R_theta, sigma, v_av, v2_av, nest])
...
def groupWrite(outfile, G_list, root):
  ...
  myTable.add_column(Column(data=empty,name='sigmaP_real', format='%0.1f'))
  myTable.add_column(Column(data=empty,name='sigmaP_mass', format='%0.1f'))
  myTable.add_column(Column(data=empty,name='M_v_mass', format='%1.2e'))
  myTable.add_column(Column(data=empty,name='R_2t_mass', format='%0.3f'))
  myTable.add_column(Column(data=empty,name='tX_mass', format='%1.2e'))  
  ...
  for i in range(0, NoGroups):  # for all groups
      for j in range(0, len(G_list[i])):  # for all galaxies
        ...
        level = galaxy.level  
        Rgroup = G_list[i][0].Rgroup  
        Rho_gr = G_list[i][0].Rho_gr  
        R_theta = G_list[i][0].R_theta  
        sigma = G_list[i][0].sigma  
        v_av = G_list[i][0].v_av  
        v2_av = galaxy.v2_av  
        ...
        sigmaP_mass = (M_v2 / (2.0E6))**(1./3)
        tX_mass = Mpc_km*R_2t2*1.05*sqrt(1.5)/sigmaP_mass/sqrt(2.5)

        

        
  
        myTable.add_row([pgc,flag,ra,dec,gl, gb, sgl,sgb,Vls,Ks,logK,B_mag, Ty, dcf2, ed, \
	  mDist, mDistErr, R_theta, sigma, sigmaP_mass, \
	    M_v2, R_2t2, tX_mass, subGalaxies, nest, \
	      coordinate_src, Ty_src , Ks_src, Vls_src, objname])
  ...
  for galaxy in galList:
      #Bol = True
      #for m in range(0, len(G_list)):
         #for j in range(1, len(G_list[m])):
            #if G_list[m][j].id == galList[p].id: 
...
def R_node(root, fraction):
  ...
  #print angular_radius/alfa, angular_radius,alfa
...
def Rgr_max(root):
  ...
  for i in range(1,N):
      ...
      dist12[i-1] = distance(root.l, root.b, root.v_av, 0, galList[i].l, galList[i].b, galList[i].Vls, galList[i].dcf2)
  
  return np.max(dist12)
...
def r_g(root):
  ...
  for i in range(1,N-1):
    for j in range(i+1,N):
      ...
      distij = distance(galList[i].l, galList[i].b, galList[i].Vls, galList[j].dcf2, galList[j].l, galList[j].b, galList[j].Vls, galList[j].dcf2 )
      sum += 1. / distij
...
def virgoPlot(table, G_list, R_min, R_max):
  ...
  for i in range(0, NoGroups):  # for all groups
	...
	if r <= 7:
	  ...
	  line.set_dashes([8, 3]) 
	  #plt.text(G_list[i][0].l, G_list[i][0].b+r, int(G_list[i][0].nest), fontsize=8, color=(Red, Blue, Green))
	...
	bool = True
        #if G_list[i][0].l<89.35 and G_list[i][0].l>82.65 and abs(G_list[i][0].b)<3.35 and G_list[i][0].nest!=37795 and G_list[i][0].nest!=38009:
	if bool:
	   r = 180*atan(H0*G_list[i][0].Rgroup/G_list[i][0].Vls)/pi
	   #plt.text(G_list[i][0].l, G_list[i][0].b+r, int(G_list[i][0].nest), fontsize=8, color=colors[col])
	   
	   Circlx = r*np.cos(theta) + G_list[i][0].l
	   Circly = r*np.sin(theta) + G_list[i][0].b
	   #line, = plt.plot(Circlx, Circly, '-', markersize = 1, color=colors[col]) 
	   #line.set_dashes([2, 3]) 
	   col+=1
	for j in range(1, len(G_list[i])):
	  #if G_list[i][j].dcf2Copy != 0:
	  
	  #if G_list[i][0].l<89.35 and G_list[i][0].l>82.65 and abs(G_list[i][0].b)<3.35 and G_list[i][0].nest!=37795 and G_list[i][0].nest!=38009:
	    #print G_list[i][0].nest 
	    sRA.append(G_list[i][j].l)
	    sDEC.append(G_list[i][j].b)
	    
	#plt.plot(sRA, sDEC, 'o', markersize = 4, color=colors[col-1], markeredgecolor = 'none')
	plt.plot(sRA, sDEC, 'o', markersize = 4, color=(Red, Blue, Green), markeredgecolor = 'none')
        
  #Circlx = 3.35*np.cos(theta) + 86.
  #Circly = 3.35*np.sin(theta) + 0.
  #line, = plt.plot(Circlx, Circly, '-', markersize = 3, color='black') 
  #line.set_dashes([3, 3]) 
...
if __name__ == '__main__':
  ...
  #ML = 60
  #if cluster == 'virgo':
    #root = readTree('virgo.ML.'+str(int(ML))+'.v07.tree')
  #elif cluster == 'fornax':
    #root = readTree('fornax.ML.'+str(int(ML))+'.v07.tree')
  #elif cluster == 'north':
    #root   = readTree('north.ML.'+str(int(ML))+'.v07.tree')
  #else:
    #root = readTree('south.ML.'+str(int(ML))+'.v07.tree')
  #print "List making ..."
  #galList = NodeLeaf(root)
  ...
  for ML in [60,61,62,63,64]:
	...
	if cluster == 'virgo':
	  ...
	  ehsan.write('virgo_dist_error.txt', format='ascii',delimiter=',')
	elif cluster == 'fornax':
	  ehsan = Table()
	  ehsan.add_column(Column(data=badDist,name='pgc'))
	  ehsan.write('fornax_dist_error.txt', format='ascii',delimiter=',')
	  
	
	if cluster == 'virgo':
	  writeTree('virgo.ML.'+str(int(ML))+'.v07.tree', root) # write the tree
	elif cluster == 'fornax':
	  writeTree('fornax.ML.'+str(int(ML))+'.v07.tree', root) # write the tree
        elif cluster == 'north':
          writeTree('north.ML.'+str(int(ML))+'.v07.tree', root) # write the tree
        else:
	  writeTree('south.ML.'+str(int(ML))+'.v07.tree', root) # write the tree
	
	
	#if cluster == 'virgo':
	  #root = readTree('virgo.ML.'+str(int(ML))+'.v06.tree')
	#elif cluster == 'fornax':
	  #root = readTree('fornax.ML.'+str(int(ML))+'.v07.tree')
        #else:
	  #root   = readTree('north.ML.'+str(int(ML))+'.v07.tree')
	  #root_1 = readTree('south.ML.'+str(int(ML))+'.v07.tree')
	...
	#G_list_1 = groupID(root_1, ML)
	  ...
	  G_list = addGalGroup(G_list, root)	
        ...
        IndivG_lis = IndividualGal(G_list, root)
	...
	galList.pop(0) 
	  ...
	  virgoPlot(table, G_list, R_min, R_max)
        ...
        if cluster == 'virgo':
	  groupWrite('virgo.ML.'+str(int(ML))+'.v07.group', G_list, root)
        elif cluster == 'fornax': 
	  groupWrite('fornax.ML.'+str(int(ML))+'.v07.group', G_list, root)
        elif cluster == 'north':
          groupWrite('north.ML.'+str(int(ML))+'.v07.group', G_list, root)
        else:
	  groupWrite('south.ML.'+str(int(ML))+'.v07.group', G_list, root)
	...
	for galaxy in galList: galaxy.inGroup = 0
        ...
        if cluster == 'virgo':
           plt.savefig('virgo.ML.'+str(int(ML))+'.v07.eps', dpi=600)  # the best view and size happens in eps format
        elif cluster == 'fornax':
	   plt.savefig('fornax.ML.'+str(int(ML))+'.v07.eps', dpi=600)  # the best view and size happens in eps format
