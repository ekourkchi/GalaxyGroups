...
if __name__ == '__main__':
   ...
   EDD = np.genfromtxt('/home/ehsan/GalGroups/start/AllSky/V8K_allsky.csv' , delimiter=',', filling_values="-100000", names=True, dtype=None )
   EDD_pgc = EDD['pgc']  # EDD
   EDD_gl_ = EDD['sgL']
   EDD_gb_ = EDD['sgB']
   gl = EDD['l']
   ...
   for i in range(len(d)):
     ...
     vh = vgsr_to_vhel(gl[m1[i]], gb[m1[i]], v_sgr[m1[i]])
     delta = abs(v[m2[i]]- vh)
     if  d[i]*3600<1 or (delta<100 and v[m2[i]]>0): #  pgc[m2[i]] == EDD_pgc[m1[i]]: #:  # EDD_Vhel[m1[i]]
       p1 += 1
       M1.append(m1[i])
       M2.append(m2[i])
       Del.append(delta)
       ...
       #if EDD_pgc[m1[i]]== 9002357:
         #print pgc[m2[i]], EDD_pgc[m1[i]], d[i]*3600, ' <> ', v[m2[i]], vh, ' <> ', d[i]*3600
   ...
   for i in range(len(PGC1)):
	 ...
	 vh_list_f.append(vh_list[index])
   ...
   myTable.add_column(Column(data=empty,name='pgc_V8K', dtype=np.dtype(int)))
   myTable.add_column(Column(data=empty,name='sgl_V8K', format='%0.4f'))
   myTable.add_column(Column(data=empty,name='sgb_V8K', format='%0.4f'))
   myTable.add_column(Column(data=empty,name='Vhel_V8K', format='%0.1f'))
   myTable.add_column(Column(data=empty,name='pgc_LEDA', dtype=np.dtype(int)))
   myTable.add_column(Column(data=empty,name='sgl_LEDA', format='%0.4f'))
   myTable.add_column(Column(data=empty,name='sgb_LEDA', format='%0.4f'))
   ...
   for i in range(len(m1_list_f)):
	 ...
	 if pgc2_list_f[i] == id:
	   flag = 1
	   break
       myTable.add_row([flag, pgc1_list_f[i], EDD_gl_[m1_list_f[i]], EDD_gb_[m1_list_f[i]], vh_list_f[i], pgc2_list_f[i], gl_[m2_list_f[i]], gb_[m2_list_f[i]], v[m2_list_f[i]], 6.*(10**logd25[m2_list_f[i]]), angle_list_f[i]])
       t +=1
       #if  pgc1_list_f[i] == 9002357:
         #print pgc1_list_f[i], vh_list_f[i], pgc2_list_f[i], v[m2_list_f[i]], angle_list_f[i]
         #print EDD_pgc[m1_list_f[i]], pgc[m2_list_f[i]]
         #print 

     
   myTable.write('V8K_LEDA_match.txt', format='ascii.fixed_width',delimiter='|', bookend=False)
   print myTable
